...
export default function UserAlbums({ isArtist }: { isArtist: boolean }) {
  const userAlbums = [
    { name: "Roadtrip Mix", img: "/artist-placeholder.jpg" },
    { name: "Chill Vibes", img: "/artist-placeholder.jpg" },
  ];
  ...
  if (!isArtist) {
    userAlbums.unshift({
      name: "Liked Songs",
      img: null!, // No image for liked songs; use icon instead
    });
  }
    ...
    <div className="mt-10 w-full max-w-4xl">
      ...
      <div className="flex gap-6 overflow-x-auto scrollbar-hide">
        {userAlbums.map((album, index) => (
          <div
            key={index}
            className="min-w-[180px] bg-gray-900 rounded-xl shadow-xl hover:scale-105

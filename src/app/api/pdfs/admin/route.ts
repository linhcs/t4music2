...
export async function POST(req: Request) {
  try {
    ...
    const genreid: idresult[] = await prisma.$queryRaw`select distinct user_id as id, username as name, min from ranking limit 50;`;

    for (let i = 0; i < genreid.length; i++) {
      const top3: genrelist[] = await prisma.$queryRaw
        `select LOWER(trim(genre)) as genre, COUNT(*) AS 'count'
                from songs 
                where genre != '' AND user_id = ${genreid[i].id}
                group by genre
                ORDER by COUNT(*) DESC 
                Limit 3;`;
    ...
    const genreprinted: boolean[] = [false, false, false, false, false, false, false, false];
    const genretype: string[] = ['Pop', 'Hip-hop', 'Rap', 'R&B', 'Rock', 'Counrty', 'EDM', 'Jazz'];
    ...
    for (let i = 1; i < infoarr.length; i++) {
      for (let j = 0; j < infoarr[i].length; j++) {
        ...
        {
          //
          if (infoarr[i][j] == genretype[k] && !genreprinted[k]) {
            page.drawText(genreid[i - 1].name, { x: 60, y: genrelocation[k], font, size: 14, });
            page.drawText(' ' + i, { x: 165, y: genrelocation[k], font, size: 14, });
            const temp: number = (genreid[i - 1].min == 'follows' ? 0 : (genreid[i - 1].min == 'likes' ? 1 : 2));

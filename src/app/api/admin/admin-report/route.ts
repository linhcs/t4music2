...
export async function GET() {
  try {
    ...
    for (const stream of streamingData) {
      ...
      if (artistId) {
        stats.artistPlays.set(artistId, (stats.artistPlays.get(artistId) || 0) + 1);
      }
    ...
    const monthlyStats = Array.from(monthlyMap.entries()).map(([month, stats]) => {
      const topGenre = Object.entries(stats.genreCount).sort((a, b) => b[1] - a[1])[0]?.[0] || 'Unknown';
      return {
        month,
    ...
    const artists = allArtists.map((artist) => {
      const play_count = artist.songs.reduce((acc, s) => acc + s.streaming_history.length, 0);
      const total_streaming_minutes = artist.songs.reduce(
        (acc, s) => acc + s.streaming_history.length * (s.duration / 60), 0);
    ...
    const totalPlayTime = Math.round(artists.reduce((acc, a) => acc + a.total_streaming_minutes, 0));
    const totalSongsAdded = artists.reduce((acc, a) => acc + a.total_songs, 0);
    const topGenre = topGenres[0]?.genre || 'Unknown';

    const monthlyTopSongs = Array.from(monthlyMap.entries()).map(([month, stats]) => {
    ...
    const monthlyTopArtists = Array.from(monthlyMap.entries()).map(([month, stats]) => {
    ...
    return NextResponse.json({
      allArtists: artists,
      topArtists: artists.sort((a, b) => b.play_count - a.play_count).slice(0, 10),

...
export default function NewSignupsTable({ period }: Props) {
  ...
  const [roleFilter, setRoleFilter] = useState<'all' | 'listener' | 'artist'>('all')
  ...
  const options: ChartOptions<'bar'> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: isAll ? 'Sign‑ups (All Months)' : `Sign‑ups in ${period!.split(' ')[1]}`,
        color: '#9932CC',
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const signupRes = await fetch('/api/adminpage/new-signups', {
        method: 'POST',
      ...
      const [listenerCounts, artistCounts] =
        roleFilter === 'all'
          ? await Promise.all([
          ...
          : roleFilter === 'listener'
          ? [await Promise.all(monthsOfYear.map((m) => fetchCount(m, 'listener'))), []]
          : [[], await Promise.all(monthsOfYear.map((m) => fetchCount(m, 'artist')))]
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          <label className="text-white">Filter by Role:</label>
          <select
            value={roleFilter}
            onChange={(e) => setRoleFilter(e.target.value as 'all' | 'listener' | 'artist')}
      ...
      <div className={`overflow-x-auto ${signups.length > 20 ? 'max-h-[640px] overflow-y-auto custom-scrollbar' : ''}`}>
          ...
          <tbody>
            {signups.map((u, i) => (
              <tr key={u.user_id} className={i % 2 ? 'bg-gray-800' : 'bg-gray-900'}>
                ...
                <td className="px-4 py-2 text-white">{new Date(u.created_at).toLocaleDateString()}</td>
              </tr>
            ))}

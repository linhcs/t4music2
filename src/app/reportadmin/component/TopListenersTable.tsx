...
export default function TopListenersTable({ period }: Props) {
  const [data, setData]       = useState<TopListener[]>([])
  const [loading, setLoading] = useState(true)
  const [chartData, setChartData] = useState<ChartData<'bar', number[], string>>({
    labels: [], datasets: [{ label:'Streamed Hours', data:[], backgroundColor:[], borderColor:[], borderWidth:2 }]
  })
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const totals: number[] = await Promise.all(
        monthsOfYear.map(async m => {
          ...
          return listeners.reduce(
            (sum: number, u: TopListener) => sum + (u.streamingHours ?? 0),
            0
          )
        })
      )
      ...
      console.error(e)
      setData([])
      setChartData({ labels:[], datasets:[{ label:'Streamed Hours',data:[],backgroundColor:[],borderColor:[],borderWidth:2 }] })
    } finally {
      setLoading(false)
    }
  },[period])
      ...
      >
        <table className="table-auto w-full text-left bg-gray-800/50 rounded-lg">
          <thead className="bg-gray-700 text-white">
            <tr>
              {['#','ID','Username','Email','Plays','Hours'].map(h=>(
